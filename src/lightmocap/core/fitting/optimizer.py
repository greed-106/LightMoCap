...
def rel_change(prev_val: float, curr_val: float) -> float:
    return (prev_val - curr_val) / max(abs(prev_val), abs(curr_val), 1.0)


def grad_require(params, flag: bool = False) -> None:
...
class FittingMonitor:
    def __init__(self, ftol: float = 1e-5, maxiters: int = 100) -> None:
        self.ftol = ftol
        self.maxiters = maxiters

    def run_fitting(self, optimizer: "LBFGS", closure, params: list[torch.Tensor]) -> float | None:
        prev_loss = None
        ...
        for iteration in range(self.maxiters):
            ...
            if torch.isnan(loss).any() or torch.isinf(loss).any():
                break
            if iteration > 0 and prev_loss is not None and rel_change(prev_loss, loss.item()) <= self.ftol:
                break
            prev_loss = loss.item()
        ...
        while n_iter < max_iter:
            ...
            if line_search_fn is not None:
                ...
                def obj_func(x, step, direction):
                    return self._directional_evaluate(closure, x, step, direction)
            ...
            if n_iter == max_iter or current_evals >= max_eval or flat_grad.abs().max() <= tolerance_grad:
                break
            if d.mul(t).abs().max() <= tolerance_change or abs(loss - prev_loss) < tolerance_change:
                break
        state["d"] = d
        state["t"] = t
